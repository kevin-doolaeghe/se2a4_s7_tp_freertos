...
void TacheCom(void *parametres) {
    while (1) {
        ...
        char d;
        char c;
        char cpt_car; // Compte le nombre de chiffres reçu à la suite
        if ((UCSR0A & (1 << RXC0)) != 0) {
            ...
            if (capt == '?') {
                ...
                d = (cpt / 10) % 10;
                c = (cpt / 100) % 10;
                UDR0 = 48 + c; // Emet centaine du compteur "cpt" si '?' est reçu
                vTaskDelay(2);
                UDR0 = 48 + d; // Emet dizaine du compteur "cpt" si '?' est reçu
                vTaskDelay(2);
                UDR0 = 48 + u; // Emet unité du compteur "cpt" si '?' est reçu
                cpt_car = 0; // On a pas reçu de nombre
            } else if (capt >= 48 && capt <= 57) {
                if (cpt_car == 0) { //Réception 1er chiffre
                    c = capt - 48;
                    cpt_car++;
                } else if (cpt_car == 1) {  //Réception 2nd chiffre
                    d = capt - 48;
                    cpt_car++;
                } else if (cpt_car == 2) {  //Réception 3ème chiffre
                    u = capt - 48;
                    cpt_car = 0; // On a reçu 3 chiffres == Réinitialisation du compteur
